...
const API = {
    ...
    API_Key : `&appid=af9eb8a327b47166f04ced688668508b`
};
...
requiredElements.generateFunction.addEventListener('click', ()  =>{
    ...
    .then((data)=>{
        ...
        postData('/postRoute', { temp : data.main.temp})
...
const postData = async ( url = ``, data = {})=>{
    ...
    console.log(`here is an error in postData function and it is : ${error}`);
    };
};
...
const updateUI = async (temperture)=>{
    try {
            requiredElements.TodayData.innerHTML = `Today 's Date : ${newDate}` 
            requiredElements.temperture.innerHTML = `the temperture is : ${temperture}`
            requiredElements.content.innerHTML = `I am feeling : ${requiredElements.feeling.value}`
    }catch(error) {
    console.log(`here is an error in updateUI function and it is : ${error}`);
    };
